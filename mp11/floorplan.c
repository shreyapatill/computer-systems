...
/*
partners: mp31, shreyap7, nbadam2
init_slicing_tree generates a slicing tree that results in a horizontally-aligned floorplan. All the modules are aligned together and 
placed along a horizontal line. The function takes a node pointer to the parent of the current recursion and an integer variable 
indicating the index on the module array. At each recursion step, we first generate an internal node with vertical cutline and a 
right child node attached by a module pointer (as the property of a left-skewed slicing tree). Then we create a left child node and 
step to the next recursion parented by the current internal node. We establish the tree links appropriately (left, right, parent 
pointers). The recursion ceases when no more modules can be attached, and ultimately returns a pointer to the tree root. 
get_total_resource takes the root pointer and returns total sum of resource of modules(the variable resource in module_t) in the tree. 
We traverse the tree to get this sum.
is_leaf_node takes a node pointer and returns 1 if the node pointer points to a leaf node in the slicing tree, or 0 otherwise. 
is_internal_node takes a node pointer and returns 1 if the node pointer points to an internal node in the slicing tree, or return 0 
otherwise.
is_in_subtree takes two node pointers to nodes a and b in the slicing tree, and should return 1 if the subtree rooted at the node b 
belongs to a part of the subtree rooted at the node a. 
get_expression traverses the slicing tree in a postfix order and encodes it into a postfix expression. The postfix expression is stored 
in the given array pointed by expression. 
postfix_traversal takes three input arguments, a node pointer pointing to a subtree from which the current recursive call takes place, 
an integer pointer indicating the position where the postfix expression needs to fill, and a pointer to the expression array. The 
postfix traversal is performed in a recursive manner. A node is processed as long as both its two children nodes are handled. 
*/
...
void swap_topology(node_t* a, node_t* b) {
  ...
  if( b_parent_temp->left == b ){
    b_parent_temp->left = a;
    a->parent = b_parent_temp;
  }else if( b_parent_temp->right == b ){
    b_parent_temp->right = a;
    a->parent = b_parent_temp;
  }
}
